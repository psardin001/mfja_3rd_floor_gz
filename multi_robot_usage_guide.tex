...
\node[flowbox, fill=flowgreen] (pose) at (6.2,-12.3) {Sample CSV polyline\\compute \texttt{x,y,z,yaw}};
...
\end{tikzpicture}}
\caption{Runtime pose computation flow. The CSV file is sampled by arc length
for geometry, while the routing table is consulted only when a segment ends.}
...
\begin{itemize}
\item network parameters: \texttt{network\_yaml}, \texttt{start\_snap\_tolerance\_m};
...
\section{Arc-length interpolation}

CSV rows are converted to a polyline. The core computes cumulative arc lengths
along that polyline. During each update, it samples the point at distance
\texttt{s}. This makes speed independent of CSV point spacing.
...
\lstinputlisting[
  ...
  lastline=92,
  firstnumber=48,
  caption={Segment geometry, cumulative length, and pose sampling.}
]{mfja_robot_control_config/scripts/room_315_kinematic_shuttle.py}

The current geometry is still polyline-based, so it follows the CSV points.
